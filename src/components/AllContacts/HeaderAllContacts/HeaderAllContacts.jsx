...
const HeaderAllContacts = ({config}) => {
  ...
  const handleInputChange = ({target: {value}}) => {
    ...
    if ('Z-A' === value) {
      dispatch(actions.getUsers('/contacts?sort%5Bby%5D=name&sort%5Border%5D=desc'));
  ...
  useEffect(() => {
    ...
    if (pathname.includes('recently-added') && textValue.length > 0) {
      const timeOutId = setTimeout(() =>
        dispatch(actions.getUsers(`/contacts?search=${textValue}&isRecently=true`)), 1000
      );
    ...
    if (pathname.includes('all-contacts') && textValue.length > 0) {
      const timeOutId = setTimeout(() =>
        dispatch(actions.getUsers(`/contacts?search=${textValue}`)), 1000
      );
      return () => clearTimeout(timeOutId);
    }
    ...
    if (pathname.includes('my-favourites') && textValue.length > 0) {
      const timeOutId = setTimeout(() =>
        dispatch(actions.getUsers(`/contacts?search=${textValue}&isFavorite=1`)), 1000
      );
      return () => clearTimeout(timeOutId);
    }
  }, [textValue]);
  ...
  const chooseGroup = ({target: {value}}) => {
    ...
    if (page === 'My favourites') {
      dispatch(actions.getUsers(`/contacts?isFavorite=1&groupId=${group[0].id}`));
    }

    if (page === 'Recently added') {
      dispatch(actions.getUsers(`/contacts?isRecently=true&groupId=${group[0].id}`));
    }
  }
          ...
          <div className="contact-book__input-inner">
            <div className="contact-book__for-select">
              ...
              <select
                ...
                className="contact-book__groups"
              >
