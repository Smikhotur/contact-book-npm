...
export const ContactDetails = ({config}) => {
  ...
  const oneUser = useSelector(
    (state) => state.reducerContactBook.oneUser
  );
  ...
  useEffect(() => {
    if (oneUser.length > 0) {
      setPerson(oneUser);
      setAddedGroup(oneUser[0].groups
        .map((group) => {
        return group.id
      }));
    } else {
      setPerson(JSON.parse(localStorage.getItem('person')));
      setAddedGroup(JSON.parse(localStorage.getItem('person'))[0].groups
        .map((group) => {
        return group.id
      }));
    }
  ...
  const sendGroups = () => {
    const remove = groups.filter((group) => {
  ...
  const removeUser = () => {
    ...
    dispatch(actions.setIdUser(JSON.parse(localStorage.getItem('person'))[0].id));
    dispatch(actions.whatToDelete(`the contact "${JSON.parse(localStorage.getItem('person'))[0].display_name}"`));
  };
