...
export const HeaderTable = ({config}) => {
  ...
  const idGroup = useSelector(
    (state) => state.reducerContactBook.idGroup
  );
  ...
  const handleInputChange = ({target: {value}}) => {
    ...
    if ('A-Z' === value) {
      dispatch(actions.getUsers(`/contacts?groupId=${idGroup}&sort%5Bby%5D=name`));
      dispatch(actions.performUnfolding(false));
    }

    if ('Z-A' === value) {
      dispatch(actions.performUnfolding(true));
      dispatch(actions.getUsers(`/contacts?groupId=${idGroup}&sort%5Bby%5D=name&sort%5Border%5D=desc`));
    }
  };
  ...
  useEffect(() => {
    ...
    if (textValue.length > 0) {
      const timeOutId = setTimeout(() =>
        dispatch(actions.getUsers(`/contacts?search=${textValue}&groupId=${idGroup}`)), 1000
      );
