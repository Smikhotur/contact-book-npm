...
export const RouterInner = ({config}) => {
  ...
  const preloader = useSelector(
    (state) => state.reducerContactBook.preloader
  );
  const message = useSelector(
    (state) => state.reducerContactBook.messageFromServer
  );
  const remove = useSelector(
    (state) => state.reducerContactBook.remove
  );
  const idUser = useSelector(
    (state) => state.reducerContactBook.idUser
  );
  ...
  const removeUser = () => {
    ...
    if (nameOfTheDeleted.includes('contacts')) {
      ...
      dispatch(actions.removeGroupContact(false));
      dispatch(actions.getAllUsers(people.filter((man) => !removeContacts.includes(man.id))));
