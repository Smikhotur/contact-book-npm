...
const TableContacts = ({idGroup, config}) => {
  ...
  const nameClass = showGroups ? 'contact-edit__list-table-groups' : 'contact-edit__list-all-groups';

  let pathname = window.location.pathname;
  const text = useSelector(
    (state) => state.reducerContactBook.searchText
  );
  ...
  const allData = useSelector(
    (state) => state.reducerContactBook.data
  );
  ...
  const checkWord = (id) => {
    return checkboxChecked.some((check) => check.name.toLowerCase() === id.toLowerCase());
  };
  ...
  const selectAllFriends = ({target: {id, checked}}) => {
      ...
      currentUsers.forEach((man) => {
        if (man.display_name[0].toLowerCase() === id.toLowerCase()) {
          if (man.groups !== undefined) {
            man.groups.forEach((group) => {
              if (!addGroup.includes(group.id) && !addedGroup.includes(group.id)) {
    ...
    if (checked === false) {
      ...
      currentUsers.forEach((man) => {
        if (idPerson.includes(man.id) && man.display_name[0].toLowerCase() !== id.toLowerCase()) {
  ...
  const checkboxHandler = ({target: {name, checked}}, man) => {
    ...
    currentUsers.forEach((parson) => {
      if (idPerson.includes(parson.id)
  ...
  const findByLetter = (letter) => {
    if (page === 'My favourites') {
      dispatch(actions
        .getUsers(`/contacts?isFavorite=1&byLetter=${letter}${idGroup[0]
          ? `&groupId=${idGroup[0].id}`
          : ''}`.trim()
        ));
      return;
    }
    ...
    if (page === 'Recently added') {
      dispatch(actions
        .getUsers(`/contacts?isRecently=true&byLetter=${letter}${idGroup[0]
          ? `&groupId=${idGroup[0].id}`
          : ''}`.trim()
        ));
      return;
    ...
    if (page === 'All contacts') {
      dispatch(actions
        .getUsers(`/contacts?byLetter=${letter}${idGroup[0]
          ? `&groupId=${idGroup[0].id}`
          : ''}`.trim()
        ));
    }
  ...
  const addAllChecked = () => {
   ...
   const listGroups = allData.data.map((man) => {
  ...
  const sendGroups = () => {
    ...
    dispatch(actions.addContactsGroups(
      sentIdPerson,
      addedGroup,
    ));
    ...
    {true ? (
      ...
      <section className="contact-book__table-block">
        <div className="contact-book__table-scroll">
          {visibleGroups && <AddingContactToGroup
          ...
          <table className="contact-book__table-wrapper">
            ...
            >
              ...
              {currentUsers.map((person, index) => {
                if (!alphabet.includes(person.display_name[0].toLowerCase())) {
                        ...
                        <td>
                          <input
                            ...
                            checked={checkboxChecked.some(
                              (check) => check.name === person.display_name[0].toUpperCase()
                            )}
                          />
                          <label
          ...
          {allData.total > 10 &&
            <div className="contact-book__block-pagination">
              ...
              <p className="contact-book__show-contacts">
                {`Showing ${1} to ${numberUser.length} of ${allData.total} contacts`}
              </p>
            </div>
          }
        </div>
